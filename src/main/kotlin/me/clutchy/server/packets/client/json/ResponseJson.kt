package me.clutchy.server.packets.client.json

data class ResponseJson(val version: VersionJson = VersionJson(), val players: PlayersJson = PlayersJson(),
                        val description: DescriptionJson = DescriptionJson(), val favicon: String = "") {

    data class VersionJson(val name: String = "1.15.2", val protocol: Int = 578)

    data class PlayersJson(val max: Int = 2, val online: Int = 1, val sample: Array<SamplePlayerJson> = arrayOf(SamplePlayerJson())) {
